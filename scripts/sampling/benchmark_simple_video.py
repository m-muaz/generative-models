...
def do_inference(accelerator, model, batch, shape, num_frames, device, uc, c, decoding_t):
    with torch.no_grad():
...
# Function to measure the latency of the model and its modules
def get_inference_time(accelerator, model, batch, shape, num_frames, device, uc, c, decoding_t, n_repeats):
    latencies = {}

    # Measure overall latency using Timer
    timer = Timer(
        stmt="do_inference(accelerator, model, batch, shape, num_frames, device, uc, c, decoding_t)",
    ...
    profile_result = timer.timeit(n_repeats)
    latencies = round(profile_result.mean * 1000, 2)  # Convert to milliseconds
    print("Overall latency: ", latencies, "ms")
    return latencies


def get_inference_memory(accelerator, model, batch, shape, num_frames, device, uc, c, decoding_t, n_repeats=1):
    if not torch.cuda.is_available():
        return 0

    # Clear the cache to get a more accurate measurement
